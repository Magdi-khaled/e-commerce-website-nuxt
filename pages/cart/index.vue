    ...
    <div class="px-5 md:px-12 py-12">
        ...
        <div>
            ...
            <div v-if="selectedSection === 0 && !show" class="sm:flex block justify-between gap-4">
                <div
                    class="xl:w-7/12 lg:w-8/12 md:w-5/12 sm:w-6/12 w-full grid lg:grid-cols-2 grid-cols-1 xl:gap-x-28 gap-x-10 gap-y-2 py-6 border-y border-y-background">
                ...
                <CartOrderSummary class="lg:w-4/12 sm:w-6/12 w-full lg:my-0 my-4" />
            </div>
                ...
                <div
                    ...
                    <div class="relative" v-for="(item, index) in xivCollection" :key="index">
                        <nuxt-link :to="`/collections/${item.id}`" class="relative">
                            <div class="relative">
                                <img :src="item.thumbnail" loading="lazy" :alt="`New Week Product (${index})`"
                                    class="m-auto lg:w-[28rem] md:w-[18rem] sm:w-[14rem] w-[16rem] lg:h-[20rem] md:h-[14rem] sm:h-[13rem] h-[12rem] border-[1px] border-neutral-300 object-cover">
                            </div>
                        ...
                        <button title="Remove from Wishlist" @click="xivCollection.splice(1, index)"
                            class="absolute right-0 lg:bottom-[21.2%] md:bottom-[28%] sm:bottom-[29%] bottom-[30%] p-1 border border-neutral-300 bg-white flex cursor-pointer text-xl text-hover hover:text-fade transition-all">
                            <Icon name="mingcute:heart-fill" size="20" class="-rotate-45" />
                        </button>
                    </div>
                </div>
            </div>
