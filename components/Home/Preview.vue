...
<template>
    <swiper :slides-per-view="1.5" :space-between="15" :navigation="{
        ...
        <swiper-slide v-for="(item, index) in items" :key="index" class="cursor-grab active:cursor-grabbing">
            <div class="relative">
                <img :src="item.thumbnail" loading="lazy" :alt="`New Week Product (${index})`" class="m-auto 2xl:w-[24rem] xl:w-[20rem] md:w-[18rem] w-[14rem] 
                2xl:h-[23rem] xl:h-[20rem] md:h-[17.5rem] sm:h-[13rem] h-[11rem] border-[1px] border-neutral-300 object-cover">
            ...
            <div class="2xl:text-lg xl:text-sm text-xs my-1 font-medium" :class="{ 'md:hidden block': isPreview }">
